    ...
    <div className="flex flex-col items-center justify-center min-h-[400px] h-full w-full py-4 px-2 font-mono select-none">
      ...
      {algorithmState.type === "merge_sort" && (
        <div className="w-full max-w-2xl flex flex-col items-center justify-center space-y-6">
            ...
            <div className="w-full flex flex-col items-center gap-4">
              <div className="flex flex-wrap items-center justify-center gap-4">
                {algorithmState.subArrays.map((sub, idx) => (
                  <div
                    key={idx}
                    className={`p-3.5 rounded-xl border transition-all duration-300 flex flex-col items-center ${
                      sub.active
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5">
                      {sub.array.map((val, vIdx) => (
                        <div
                          key={vIdx}
                          className={`w-11 h-11 rounded-lg border flex items-center justify-center font-bold text-sm shadow-inner transition-all ${
                            sub.active
          ...
          {algorithmState.comparison && (
            <div className="w-full max-w-lg p-4 rounded-xl bg-[#141416] border border-amber-500/40 shadow-lg shadow-amber-500/5 space-y-2 animate-in fade-in zoom-in-95">
              ...
              <div className="flex items-center justify-center gap-4 text-base font-bold text-[#F4F4F5] py-1">
                <div className="flex flex-col items-center">
                  <span className="text-[10px] text-[#71717A] mb-0.5">Left</span>
                  <span className="px-3.5 py-1.5 rounded-lg bg-blue-500/20 text-blue-300 border border-blue-500/40">
                    {algorithmState.comparison.left}
                  </span>
                </div>

                <span className="text-xl text-amber-400 font-black">{algorithmState.comparison.op}</span>

                <div className="flex flex-col items-center">
                  <span className="text-[10px] text-[#71717A] mb-0.5">Right</span>
                  <span className="px-3.5 py-1.5 rounded-lg bg-indigo-500/20 text-indigo-300 border border-indigo-500/40">
            ...
            <div className="w-full flex flex-col items-center space-y-2 pt-2 animate-in fade-in">
              ...
              <div className="flex items-center justify-center gap-2 flex-wrap">
                {algorithmState.merged.map((val, idx) => (
                  <div
                    key={idx}
                    className="w-12 h-12 rounded-xl bg-emerald-950/40 border-2 border-emerald-500/60 text-emerald-300 font-extrabold text-base flex items-center justify-center shadow-lg shadow-emerald-500/10 scale-105"
                  >
                    {val}
                  </div>
                ))}
      ...
      {algorithmState.type === "binary_search" && algorithmState.array && (
        <div className="w-full max-w-2xl flex flex-col items-center justify-center space-y-8">
          ...
          <div className="w-full overflow-x-auto pb-4 pt-6 flex justify-center">
            <div className="flex items-end gap-2.5 min-w-max">
              {algorithmState.array.map((val, idx) => {
                ...
                return (
                  <div key={idx} className="flex flex-col items-center gap-2">
                    ...
                    <div className="h-7 flex items-center justify-center">
                      {pointer ? (
                        <div
                          ...
                          className="text-[10px] font-bold px-2 py-0.5 rounded-full border bg-[#18181B] shadow-md flex items-center gap-1 animate-bounce"
                        >
                    ...
                    {/* Array Cell */}
                    <div
                      className={`w-14 h-14 rounded-xl border flex flex-col items-center justify-center text-base font-extrabold transition-all duration-300 shadow-sm ${
                    ...
                    {/* Index Label */}
                    <span className="text-[11px] text-[#71717A] font-bold">[{idx}]</span>
                  </div>
                );
              })}
            </div>
